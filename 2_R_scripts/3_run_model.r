...
likelihood_lag <- function(pars, data, conditions) {
    ...
    for (i in 1:length(scaled_base)) {
        lag <- scaled_base[i]
        m_results[, i] <- dnorm(growth_curve_lag(pars, data, lag) - data$agbd, sd = pars["sd"])
    }
...
# growth_curve(pars, df_eu, 1)
# tst <- dnorm(growth_curve(pars, data, lag) - data$agbd, sd = pars["sd"])
# class(tst)
...
# for (i in 1:length(scaled_base)) {
#     lag <- scaled_base[i]
#     m_results[, i] <- dnorm(growth_curve(pars, data, lag) - data$agbd, sd = pars["sd"])
# }

# # Calculate log-likelihood
# result <- sum(-log(rowMeans(m_results)))
# result
# -log(rowMeans(m_results))
# rowMeans(m_results)
