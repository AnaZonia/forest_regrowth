...
I noticed that if I try to add a specific standard deviation value as the last element
in the pars vector, predictions are way worse, which is understandable, but will become relevant
further ahead:
...
Both CWD and Evapotranspiration can be derived from precipitation data (this comes from TerraClimate):

```{r}
knitr::include_graphics("terraclimate.jpg")
```
