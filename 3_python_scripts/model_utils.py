...
#     """
#     B0, theta, k0 = params[:3]
#     coeffs = params[3:]

#     # Convert X to a regular 2D numpy array
#     X = np.array([X[col_name] for col_name in X.dtype.names])
#     k = np.dot(X.T, coeffs) + k0
#     k = np.where(k < 0, -np.log(1 - (y.mean() / A.mean())), k)
...
def nelder_mead_lag(params, X, y, A, fold = 1, return_predictions = False):
    ...
    pars_coeffs = np.squeeze(X_array * coeffs)
...
def process_fold(X, y, A, params, kf, nelder_mead_func, fold):
    ...
    X_train_scaled = pd.DataFrame(scaler.fit_transform(X_train),
                                     columns=X_train.columns, index=X_train.index).to_records(index=False)
